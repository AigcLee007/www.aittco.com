...
async function handleNotify(req: Request, method: 'GET' | 'POST') {
  try {
    ...
    console.info('[payment][wechat][notify] received', {
      method,
    ...
    if (!payload.orderNo) {
      console.warn('[payment][wechat][notify] rejected: missing orderNo');
      return new Response('missing orderNo', { status: 400 });
    }

    if (expectedAppId && payload.appId && payload.appId !== expectedAppId) {
      console.warn('[payment][wechat][notify] rejected: invalid appid', { expectedAppId, got: payload.appId });
      return new Response('invalid appid', { status: 400 });
    }

    if (payload.pid && expectedPid && payload.pid !== expectedPid) {
      console.warn('[payment][wechat][notify] rejected: invalid pid', { expectedPid, got: payload.pid });
      return new Response('invalid pid', { status: 400 });
    }

    if (payload.tradeStatus !== 'TRADE_SUCCESS' && payload.tradeStatus !== 'OD') {
      console.warn('[payment][wechat][notify] rejected: invalid trade status', { tradeStatus: payload.tradeStatus });
      return new Response('invalid trade_status', { status: 400 });
    }

    if (payload.paymentType && payload.paymentType !== 'wxpay' && payload.paymentType !== 'wechat') {
      console.warn('[payment][wechat][notify] rejected: invalid payment type', { paymentType: payload.paymentType });
    ...
    if (payload.sign && (verifyXunhuSign(signable, payload.sign) || verifyZPaySign(signable, payload.sign))) {
      ...
      console.warn('[payment][wechat][notify] rejected: invalid signature and notify token');
    ...
    console.info('[payment][wechat][notify] settled', { orderNo: payload.orderNo, transactionId: payload.transactionId });

    return new Response('success', { status: 200 });
  } catch (error: any) {
    console.error('[payment][wechat][notify] failed', error);
...
export async function GET(req: Request) {
  ...
  return NextResponse.json({ ok: true, channel: 'WECHAT', endpoint: '/api/payments/wechat/notify' });
}
