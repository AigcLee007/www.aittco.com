...
import { MathWelcome } from './math-welcome/MathWelcome';
...
export function ChatMessageList(props: {
  ...
  // [Scroll] scroll to the very bottom of a new chat or when a new assistant message starts
  const lastMessageId = conversationMessages.length ? conversationMessages[conversationMessages.length - 1].id : null;
  const lastMessageIsStreaming = conversationMessages.length ? (conversationMessages[conversationMessages.length - 1].role === 'assistant' && conversationMessages[conversationMessages.length - 1].pendingIncomplete) : false;
  ...
  }, [lastMessageId, lastMessageIsStreaming, setStickToBottom]);
  ...
  if (!filteredMessages.length)
    return (
      <Box sx={{ ...props.sx, display: 'flex', flexDirection: 'column', flex: 1, minHeight: '100%', backgroundColor: 'transparent' }}>
        <MathWelcome composer={props.composer} />
      </Box>
    );
  ...
  return (
    <List role='chat-messages-list' sx={listSx} onCopy={clipboardInterceptCtrlCForCleanup}>
      ...
      {filteredMessages.map((message, idx) => {
          ...
          // 统一使用 ChatMessageMemo，由于在生成时 message 引用的高频改变会导致 re-render 照常触发（这是预期的进度条表现），
          // 而在生成完成后，保持组件引用相同非常关键，这能防止 React 销毁重建这棵带有打字机本地状态 DOM 树！从而让残存的打字机特效继续平滑流出。
