...
function WebInputModal(props: {
  ...
  const handleSubmit = React.useCallback(({ links }: WebInputModalInputs) => {
    ...
    if (!cleanUrls.length) {
      addSnackbar({ key: 'invalid-urls', message: '请输入至少一个有效的网址', type: 'issue', overrides: { autoHideDuration: 2000 } });
      return;
    }
  ...
  return (
    ...
    >
      ...
      <Typography level='body-sm'>
        支持大多数网站；对于 YouTube 视频（例如 youtube.com/...），将导入字幕。
      ...
      <form onSubmit={formHandleSubmit(handleSubmit)}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', gap: 1, mt: 2.5 }}>
          ...
          >
            导入 {urlFieldCount > 1 ? `(${urlFieldCount})` : ''}
          </Button>
