...
export const DocAttachmentFragmentPane = React.memo(function DocAttachmentFragment(props: {
  ...
  const { liveFileControlButton, liveFileActions } = useLiveFileSync(
    ...
    workspaceId,
    props.isMobile,
    'text' in fragmentDocPart.data ? fragmentDocPart.data.text : '',
  ...
  const viewAsLabel =
    !viewAsCode ? (fragmentDocPart.vdt ? 'text' : '(unknown)')
      : (('mimeType' in fragmentDocPart.data && fragmentDocPart.data.mimeType && fragmentDocPart.data.mimeType !== fragmentDocPart.vdt) ? fragmentDocPart.data.mimeType : '');
  ...
  const headerTooltipContents = React.useMemo(() => (
    <Box sx={enhancedCodePanelTitleTooltipSx}>
      ...
      <div>{('mimeType' in fragmentDocPart.data ? fragmentDocPart.data.mimeType : undefined) || '(unknown)'}</div>
      <div>Render type</div>
  ...
  return (
    ...
    >
      ...
      {('reftype' in fragmentDocPart.data) ? (
        <Box sx={{ p: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 2, border: '1px dashed', borderColor: 'neutral.outlinedBorder', borderRadius: 'sm', color: 'text.tertiary' }}>
          <Typography level='title-sm'>
             Binary Attachment ({'mimeType' in fragmentDocPart.data ? fragmentDocPart.data.mimeType : 'unknown'})
          </Typography>
        </Box>
      ) : isEditing ? (
        // Document Editor
        ...
        <Box py={1}>
          <AutoBlocksRenderer
            ...
            // text={marshallWrapText(fragmentDocPart.data.text, /*part.meta?.srcFileName || part.ref*/ undefined, 'markdown-code')}
            text={'text' in fragmentDocPart.data ? fragmentDocPart.data.text : ''}
            renderAsCodeWithTitle={viewAsCode ? (('mimeType' in fragmentDocPart.data ? fragmentDocPart.data.mimeType : undefined) || fragmentDocPart.ref || fragmentTitle) : undefined}
