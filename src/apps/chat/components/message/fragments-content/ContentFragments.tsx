...
export function ContentFragments(props: {
  ...
  return <Box aria-label='message body' sx={(showDataStreamViz || isEditingText) ? _editLayoutSx : fromAssistant ? _startLayoutSx : _endLayoutSx}>
    ...
    {/* Thinking Indicator for AI (Deep Thinking) */}
    {fromAssistant && isEmpty && props.messagePendingIncomplete && (
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, py: 0.5 }}>
        <Box sx={{ display: 'flex', position: 'relative' }}>
          <CircularProgress size="sm" thickness={2} variant="soft" />
        </Box>
        <Typography level="body-sm" sx={{ fontStyle: 'italic', color: 'text.secondary', animation: 'pulse 2s infinite' }}>
          深度思考中...
        </Typography>
      </Box>
    )}
