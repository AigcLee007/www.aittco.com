...
export function ViewDocPartModal(props: {
  ...
  const mimeType = ('mimeType' in docPart.data ? docPart.data.mimeType : undefined) || '(unknown)';
      ...
      <Box sx={containerSx}>
        ...
        <Box color='primary' sx={{ px: 1.5, fontSize: 'sm' }}>
          <Box sx={propGridSx}>
            ...
            <div>{docPart.ref}</div>
            <div>Mime Type</div>
            <div>{('mimeType' in docPart.data ? docPart.data.mimeType : undefined) || '(unknown)'}</div>
            <div>Render Type</div>
            <div>{docPart.vdt}</div>
        ...
        <Box sx={textPageSx}>
          {'text' in docPart.data ? (
            ...
            <Box sx={{ p: 2, color: 'text.tertiary', textAlign: 'center' }}>
