  ...
  return (
    ...
    >
      ...
      {isMultiInstance && hasInstances && canAdd && (
        <GoodTooltip title={`添加另一个 ${vendor.name.replace('OpenAI', 'OpenAI 兼容')} 服务`} placement='top' arrow>
          <IconButton
            size='sm'
            color='primary'
            variant='soft'
            tabIndex={0}
            aria-label={`添加另一个 ${vendor.name} 服务`}
            onClick={(e) => {
              e.stopPropagation();
...
export function ModelsServiceSelector(props: {
  ...
  const vendorComponents = React.useMemo(() => {
    ...
    const components: React.ReactNode[] = [
      ...
      // _renderSectionHeader('云端'),
      // ...vendorItems
      //   .filter(v => v.vendor.displayGroup === 'cloud')
      //   .sort((a, b) => a.vendor.name.localeCompare(b.vendor.name))
      //   .map(item => _renderVendorItem(item, isMobile, handleAddServiceForVendor)),

      // _renderSectionHeader('本地'),
      // ...vendorItems
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', gap: 1 }}>
      ...
      {(isMobile && !noServices) ? (
        ...
        <Tooltip open={noServices && !vendorsMenuAnchor} variant='outlined' color='primary' size='md' placement={isMobile ? 'bottom-end' : 'top-start'} arrow title='添加您的第一个 AI 服务'>
      ...
      {enableDeleteButton && (
        <TooltipOutlined title={`移除 ${selectedServiceItem?.service.label || '服务'}`}>
          <IconButton
