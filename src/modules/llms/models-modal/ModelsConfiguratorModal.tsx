...
export function ModelsConfiguratorModal(props: {
  ...
  const handleRemoveClones = React.useCallback(() => {
    showPromisedOverlay('llms-remove-clones', {}, ({ onResolve, onUserReject }) =>
      <ConfirmationModal
        open onClose={onUserReject} onPositive={() => onResolve(true)}
        confirmationText={`确定要从 ${activeService?.label ?? '此服务'} 中移除所有用户克隆的模型吗？`}
        positiveActionText='移除'
      />,
  ...
  const startButton = React.useMemo(() => {
    if (isTabWizard)
      return <Button variant='outlined' color='neutral' onClick={handleShowAdvanced} sx={{ backgroundColor: 'background.popup' }}>{isMobile ? '更多服务' : '更多服务'}</Button>;
    ...
    if (!hasAnyServices)
      return <Button variant='outlined' color='neutral' onClick={handleShowWizard} sx={{ backgroundColor: 'background.popup' }}>{isMobile ? '快速设置' : '快速设置'}</Button>;
        ...
        <Dropdown>
          ...
          <Menu placement='bottom-start' disablePortal sx={{ minWidth: 240 }}>
            ...
            <MenuItem disabled={false /* */} onClick={handleResetAllParameters}>
              <ListItemDecorator><SettingsBackupRestoreIcon /></ListItemDecorator>
              重置个性化设置
            </MenuItem>

            {/* Remove Cloned Models */}
            <MenuItem disabled={false /* unnecessary: !hasClones */} onClick={handleRemoveClones}>
              <ListItemDecorator><AutoFixHighIcon /></ListItemDecorator>
              移除重复模型
            </MenuItem>
            ...
            <MenuItem onClick={handleResetVisibility}>
              <ListItemDecorator><UndoIcon /></ListItemDecorator>
              重置可见性
            </MenuItem>
            ...
            <MenuItem onClick={() => setShowModelsHidden(!showModelsHidden)}>
              ...
              {showModelsHidden ? '隐藏已隐藏的模型' : '显示已隐藏的模型'}
            </MenuItem>
      ...
      <Box sx={{ display: 'flex', width: '100%', gap: 1, justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        {hasUnsavedChanges && (
          <Typography color='warning' level='body-sm' ml='auto'>
            {isMobile ? '未保存' : `您有 ${unsavedWizardProviders.size} 项未保存的更改`}
          </Typography>
        )}
  ...
  if (showExplainer) {
    return (
      <GoodModal
        title={
          <AppBreadcrumbs size='md' rootTitle='欢迎'>
            <AppBreadcrumbs.Leaf>关于关联 AI 服务的通知</AppBreadcrumbs.Leaf>
  ...
  return (
    <GoodModal
      title={isTabWizard ? (
        <AppBreadcrumbs size='md' rootTitle='欢迎'>
          <AppBreadcrumbs.Leaf><b>设置 AI 模型</b></AppBreadcrumbs.Leaf>
        </AppBreadcrumbs>
      ) : (
        // <>Configure <b>AI Models</b></>
        <AppBreadcrumbs size='md' rootTitle='配置'>
      ...
      closeText={isTabWizard ? '完成' : undefined}
