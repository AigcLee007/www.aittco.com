  ...
  createImages: edgeProcedure
    ...
    .mutation(async function* ({ input, signal }): AsyncGenerator<T2ICreateImageAsyncStreamOp> {
      ...
      if (!isEdit) {
        ...
        requestBody.append('user', user || 'AIGC-Club');
      ...
      for (const image of images) {
        if (!('b64_json' in image)) {
          // [OpenAI Issue] Falls back to URL download if b64_json is missing
          if ('url' in image && typeof image.url === 'string') {
            try {
              const imageResponse = await fetch(image.url);
              if (imageResponse.ok) {
                const arrayBuffer = await imageResponse.arrayBuffer();
                image.b64_json = Buffer.from(arrayBuffer).toString('base64');
              } else {
                console.error(`openai.router.createImages: failed to download image from url`, { status: imageResponse.status });
              }
            } catch (error) {
              console.error(`openai.router.createImages: failed to download image`, error);
            }
          }
