...
const openAIModelsDenyList: string[] = [
  ...
  'image', // [2026-02-09] Filter out all image models per user request
  'banana', // [2026-02-09] Filter out "banana" models (Gemini Nano)
...
const openAIModelsAllowList = [
  'gpt', 'chatgpt', 'o1', 'o3', 'o4', 'o5', 'dall', 'whisper', 'tts', 'omni-moderation', 'text-embedding', 'text-moderation', 'grok',
];

export function openAIModelFilter(model: OpenAIWire_API_Models_List.Model) {
  // 1. Deny list check
  if (openAIModelsDenyList.some(deny => model.id.includes(deny))) return false;

  // 2. Allow list check (User request: "OpenAI 接口只能出来GPT模型列表" - meaning official only)
  const idLower = model.id.toLowerCase();
  return openAIModelsAllowList.some(allow => idLower.startsWith(allow));
}
