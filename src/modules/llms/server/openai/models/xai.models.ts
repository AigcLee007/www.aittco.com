...
// Reasoning variants have configuration only for grok-3-mini
const XAI_PAR_Reasoning_Mini: ModelDescriptionSchema['parameterSpecs'] = [
  ...XAI_PAR,
...
const _knownXAIChatModels: ManualMappings = [
  ...
  },
  {
    ...
    parameterSpecs: XAI_PAR_Reasoning_Mini,
...
export async function xaiFetchModelDescriptions(access: OpenAIAccessSchema): Promise<ModelDescriptionSchema[]> {
  ...
  return xaiModels.models.reduce((acc, xm) => {
    ...
    const unknownModelFallback: KnownModel = {
      idPrefix: xm.id,
      label: modelLabelTitleCase(_xaiFormatNewModelLabel(xm.id)),
