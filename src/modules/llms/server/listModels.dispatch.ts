...
// protocol: Anthropic
import { anthropicInjectVariants, anthropicModelFilter, anthropicValidateModelDefs_DEV, AnthropicWire_API_Models_List, hardcodedAnthropicModels, llmsAntCreatePlaceholderModel } from './anthropic/anthropic.models';
    ...
    case 'anthropic': {
      return createDispatch({
        ...
        convertToDescriptions: (wireModelsResponse) => {
          ...
          // filter based on accepted models (official only)
          const filteredModels = availableModels.filter(anthropicModelFilter);
    ...
    case 'gemini': {
      return createDispatch({
        fetchModels: async () => {
          ...
          // [Debug] Log raw response from relay
          console.log('[Gemini Dispatch] Raw Response:', JSON.stringify(wireModels, null, 2));
      ...
      return createDispatch({
        ...
        convertToDescriptions: (detailedModels) => {
          return detailedModels.map((model) => {
            ...
            const label = modelLabelTitleCase(modelName) + ((modelTag && modelTag !== 'latest') ? ` (${modelTag})` : '');
    ...
    case 'togetherai':
      return createDispatch({
        ...
        fetchModels: async () => {
          ...
          _wire?.logRequest('GET', url, headers);
          const wireModels = await fetchJsonOrTRPCThrow<OpenAIWire_API_Models_List.Response>({ url, headers, name: `OpenAI/${modelLabelTitleCase(dialect)}`, signal });
          _wire?.logResponse(wireModels);
          return wireModels;
        },
