import { DModelInterfaceV1, LLM_IF_Outputs_Image, LLM_IF_Tools_WebSearch } from '~/common/stores/llms/llms.types';
...
export function fromManualMapping(mappings: (KnownModel | KnownLink)[], upstreamModelId: string, created: undefined | number, updated: undefined | number, fallback: KnownModel, disableSymlinkLooks?: boolean): ModelDescriptionSchema {
  ...
  // build label (a bit tricky)
  let label = modelLabelTitleCase(m.label);
  let description = m.description || '';

  // [User Request] "Exact naming as transfer station" for unknown/variant models
  if (resolution === 'super' || resolution === 'fallback') {
    label = modelLabelTitleCase(upstreamModelId);
    delete m.hidden;
  } else if (!disableSymlinkLooks && symlinkTarget) {
    // add a symlink icon to the label
    label = `🔗 ${label} → ${symlinkTarget}`;
