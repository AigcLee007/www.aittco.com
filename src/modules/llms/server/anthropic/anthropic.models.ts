...
const anthropicModelsAllowList = ['claude', 'anthropic'];

export function anthropicModelFilter(model: AnthropicWire_API_Models_List.ModelObject) {
  // User request: "Anthropic 接口：只显示名字里带 claude (or anthropic) 的模型"
  const nameLower = (model.display_name || model.id).toLowerCase();
  return anthropicModelsAllowList.some(keyword => nameLower.includes(keyword));
}
...
export function llmsAntCreatePlaceholderModel(model: AnthropicWire_API_Models_List.ModelObject): ModelDescriptionSchema {
  return {
    ...
    label: modelLabelTitleCase(model.display_name),
...
export function llmOrtAntLookup_ThinkingVariants(orModelName: string): OrtVendorLookupResult | undefined {
  ...
  return {
    interfaces,
    parameterSpecs,
    labelOverride: model.labelOverride || model.label,
  };
}
