    ...
    <Box sx={props.isMobile ? rayGridMobileSx : rayGridDesktopSx}>
      ...
      {props.rayIds.map((rayId, index) => (
        <BeamRay
          ...
          rayId={rayId}
          sx={{
            flex: props.isMobile ? '1 1 100%' : '1 1 400px',
            minWidth: props.isMobile ? undefined : '360px',
          }}
      ...
      {(props.showRayAdd && raysCount < SCATTER_RAY_MAX) && (
        <BeamCard sx={{ mb: 'auto', p: 0.5, display: 'flex', alignItems: 'center', justifyContent: 'center', width: 'fit-content', flexShrink: 0 }}>
      ...
      {!!props.showRaysOps && (
        <Box sx={{ flex: '1 0 100%', display: 'flex', justifyContent: 'center', gap: 2, mt: 2 }}>
