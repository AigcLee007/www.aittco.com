  ...
  return (
    <Modal open={props.open} onClose={handleClose}>
      <ModalDialog>
        ...
        <DialogContent>存储模型配置。</DialogContent>
  ...
  return <>
    ...
    <Dropdown>
      ...
      <Menu placement='right-end' sx={{ minWidth: 200, zIndex: 'var(--joy-zIndex-modal)' /* on top of its own modal in FS */ }}>
        <ListItem>
          <Typography level='body-sm'>模型预设</Typography>
        </ListItem>
        ...
        {presets.map(preset =>
          <MenuItem key={preset.id} onClick={() => handlePresetLoad(preset.id)}>
            <ListItemDecorator />
            <Typography>
              加载 &quot;{preset.name}&quot; &nbsp;<span style={{ opacity: 0.5, marginRight: '2rem' }}>x{preset.rayLlmIds?.length}</span>
            </Typography>
            <IconButton
              size='sm'
              variant='outlined'
        ...
        <ListItem>
          <Typography level='body-sm'>视图</Typography>
        </ListItem>
        ...
        <ListItem onClick={DEV_MODE_SETTINGS ? () => handleClearLastConfig() : undefined}>
          <Typography level='body-sm'>高级</Typography>
        </ListItem>
