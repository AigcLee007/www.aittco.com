...
export class ChatGenerateTransmitter implements IParticleTransmitter {
  ...
  private isExtractingTitle: boolean | undefined = undefined;
  ...
  * flushParticles(): Generator<AixWire_Particles.ChatGenerateOp> {
  ...
  /**
   * Support function to extract:
   * 1. potential reasoning text in between <think> and </think> tags
   * 2. potential conversation title in between <title> and </title> tags
   * if and only if they're at the very beginning of the whole session.
   */
  appendAutoText_weak(textChunk: string) {
    // fast-path: if we are done with all the weak extractions, just append
    if (this.isThinkingText === false && this.isExtractingTitle === false) {
    ...
    // Phase 1: Initialize detection if needed
    const isAtBeginning = this.isThinkingText === undefined && this.isExtractingTitle === undefined;
    if (isAtBeginning) {
      const trimmed = remaining.trimStart();
      if (trimmed.startsWith('<think>')) {
        this.isThinkingText = true;
        this.isExtractingTitle = false; // logic: if it's think, it's not title
        remaining = trimmed.substring('<think>'.length);
      } else if (trimmed.startsWith('<title>')) {
        this.isExtractingTitle = true;
        this.isThinkingText = false; // logic: if it's title, it's not think
        remaining = trimmed.substring('<title>'.length);
      } else {
    ...
    while (remaining.length > 0) {
      ...
      if (this.isThinkingText) {
        ...
        if (closingIdx >= 0) {
          ...
          this.isThinkingText = false;
          remaining = remaining.substring(closingIdx + '</think>'.length).trimStart();
          // check if title follows immediately? (unlikely, but let's be robust)
          if (remaining.startsWith('<title>')) {
            this.isExtractingTitle = true;
            remaining = remaining.substring('<title>'.length);
          }
          continue;
        } else {
      ...
      // 2b. Extraction: Title
      if (this.isExtractingTitle) {
        const closingIdx = remaining.indexOf('</title>');
        if (closingIdx >= 0) {
          const titleText = remaining.substring(0, closingIdx);
          if (titleText.trim())
            this.sendControl({ cg: 'set-title', title: titleText.trim() });
          this.isExtractingTitle = false;
          remaining = remaining.substring(closingIdx + '</title>'.length).trimStart();
          // check if think follows immediately?
          if (remaining.startsWith('<think>')) {
            this.isThinkingText = true;
            remaining = remaining.substring('<think>'.length);
          }
          continue;
        } else {
          // Note: we don't stream title chars for now, we wait for the closing tag
          // or we could accumulate them if needed. For simplicity: buffer till end?
          // Actually, let's just wait for the closing tag.
          // If the chunk is very large, this might be an issue, but titles are short.
          // For now, we'll just drop the text from the main stream.
          return;
        }
      }

      // 2c. Regular text
      const nextThinkIdx = remaining.indexOf('<think>');
      const nextTitleIdx = remaining.indexOf('<title>');
      // Only extract tags if they are at the "beginning" of remaining (after think/title removal)
      // but if they appear in the middle of text, we usually don't extract them here to avoid false positives.
