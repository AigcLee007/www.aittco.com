...
export class ContentReassembler {
  ...
  private titleScanActive = true;
  private titleScanBuffer = '';
  ...
  private onAppendText(particle: AixWire_Particles.TextParticleOp): void {
    const safeTextChunk = this._extractLeadingTitleAndStripFromText(particle.t);
    if (!safeTextChunk)
      return;
    ...
    if (currentTextFragment && isTextContentFragment(currentTextFragment)) {
      currentTextFragment.part.text += safeTextChunk;
      return;
    }

    // new TextContentFragment
    const newTextFragment = createTextContentFragment(safeTextChunk);
  ...
  /**
   * Final client-side safety net for leaked "<title>...</title>" blocks.
   * It only runs at the beginning of a message stream and strips the title from visible text,
   * while still assigning it to `genTitle`.
   */
  private _extractLeadingTitleAndStripFromText(textChunk: string): string {
    if (!this.titleScanActive)
      return textChunk;

    this.titleScanBuffer += textChunk;
    const probe = this.titleScanBuffer.trimStart();
    const titleOpenTag = '<title>';
    const isOpenTagPrefix = titleOpenTag.startsWith(probe);

    // Handle split chunks, e.g. "<ti" + "tle>..."
    if (probe.length > 0 && isOpenTagPrefix && !probe.startsWith(titleOpenTag))
      return '';

    if (!probe.startsWith(titleOpenTag)) {
      this.titleScanActive = false;
      const passthrough = this.titleScanBuffer;
      this.titleScanBuffer = '';
      return passthrough;
    }

    const closingIdx = probe.indexOf('</title>');
    if (closingIdx < 0)
      return '';

    const parsedTitle = probe.slice('<title>'.length, closingIdx).trim();
    if (parsedTitle)
      this.accumulator.genTitle = parsedTitle;

    const remaining = probe.slice(closingIdx + '</title>'.length).trimStart();
    this.titleScanBuffer = '';
    this.titleScanActive = false;
    return remaining;
  }
  ...
  private onSetTitle({ title }: Extract<AixWire_Particles.ChatGenerateOp, { cg: 'set-title' }>): void {
    this.accumulator.genTitle = title;
  }
