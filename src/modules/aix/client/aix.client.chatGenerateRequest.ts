...
import { getDocumentAsset, getImageAsset } from '~/common/stores/blob/dblobs-portability';
...
import { DMessageDocPart, DMessageFragment, DMessageImageRefPart, DMessageZyncAssetReferencePart, isContentOrAttachmentFragment, isToolResponseFunctionCallPart, isVoidThinkingFragment } from '~/common/stores/chat/chat.fragments';
...
import type { AixAPIChatGenerate_Request, AixMessages_ModelMessage, AixMessages_ToolMessage, AixMessages_UserMessage, AixParts_DocPart, AixParts_InlineImagePart, AixParts_MetaCacheControl, AixParts_MetaInReferenceToPart, AixParts_ModelAuxPart } from '../server/api/aix.wiretypes';
    ...
    switch (sFragment.ft) {
      ...
      case 'attachment':
        switch (sFragment.part.pt) {
          ...
          case 'doc':
            try {
              sm.parts.push(await aixConvertDocPartToAixDocPartOrThrow(sFragment.part));
            } catch (error) {
              console.warn('[DEV] aixCGR_SystemMessage: skipping doc part', error);
            }
            break;
...
async function aixConvertDocPartToAixDocPartOrThrow(docPart: DMessageDocPart): Promise<AixParts_DocPart> {
  // 1. Inline Text: Pass through
  if ('idt' in docPart.data && docPart.data.idt === 'text')
    return docPart as unknown as AixParts_DocPart;

  // 2. DBlob Reference: Resolve to base64
  if ('reftype' in docPart.data && docPart.data.reftype === 'dblob') {
    const docAsset = await getDocumentAsset((docPart.data as any).dblobAssetId);
    if (!docAsset) throw new Error('Document asset not found');
    return {
      ...docPart,
      data: {
        idt: 'base64',
        base64: docAsset.data.base64,
        mimeType: docAsset.data.mimeType,
      },
    } as unknown as AixParts_DocPart;
  }

  // 3. Other references (e.g. URL): Not supported yet
  throw new Error('Unsupported DocPart data type or reference');
}
    ...
    if (dMessageRole === 'user') {
      ...
      const aixChatMessageUser = await dMessageUserFragments.reduce(async (uMsgPromise, uFragment: DMessageFragment) => {
        ...
        switch (uFragment.part.pt) {
          ...
          case 'doc':
            try {
              uMsg.parts.push(await aixConvertDocPartToAixDocPartOrThrow(uFragment.part));
            } catch (error) {
              console.warn('[DEV] aixCGR_ChatSequence: skipping doc part', error);
            }
            break;
