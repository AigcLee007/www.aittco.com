...
export function parseBlocksFromText(text: string): RenderBlockInputs {
  ...
  const regexPatterns = {
    ...
    // codeBlock: /^( {0,3})`{3,}([^\n`]*)\n([\s\S]*?)(?:\n^\1`{3,}[^\S\n]*(?=\n|$))?/gm,
    codeBlock: /`{3,}([^\n`]*)\n([\s\S]*?)(`{3,}(?=[ *\n]|$)|$)/g,
    htmlCodeBlock: /<(html|head|body|script|style)(?:\s[^>]*)?>([\s\S]*?)<\/\1>/gi,
  ...
  while (true) {
    ...
    switch (matchType) {
      ...
      case 'htmlCodeBlock':
        const preMatchHtml: string = match[0];
        blocks.push({ bkt: 'code-bk', title: match[1].toLowerCase(), code: preMatchHtml, lines: countLines(preMatchHtml), isPartial: false });
        break;
