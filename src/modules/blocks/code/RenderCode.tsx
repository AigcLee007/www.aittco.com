          ...
          <Box sx={overlayFirstRowSx}>
            ...
            {isHTMLCode && (
              <ButtonGroup aria-label='HTML options' sx={overlayGroupWithShadowSx}>
                <OverlayButton tooltip={noTooltips ? null : renderHTML ? '显示代码' : '显示网页'} variant={renderHTML ? 'solid' : 'outlined'} color='danger' onClick={() => setShowHTML(!showHTML)}>
                  <HtmlIcon sx={{ fontSize: 'xl2' }} />
                </OverlayButton>
                {renderHTML && (
                  <OverlayButton tooltip={noTooltips ? null : '重新加载'} variant='outlined' color='danger' onClick={() => setHtmlReloadKey(k => k + 1)}>
                    <ReplayRoundedIcon />
                  </OverlayButton>
                )}
              </ButtonGroup>
              ...
              <ButtonGroup aria-label='Diagram' sx={overlayGroupWithShadowSx}>
                ...
                <OverlayButton
                  tooltip={noTooltips ? null
                    : (renderSVG || renderMermaid || renderPlantUML) ? '显示代码'
                      : isSVGCode ? '渲染SVG'
                        : isMermaidCode ? 'Mermaid图表'
                          : 'PlantUML图表'
                  }
                ...
                {((isMermaidCode && showMermaid) || (isPlantUMLCode && showPlantUML && !plantUmlError) || (isSVGCode && showSVG && canScaleSVG)) && (
                  <OverlayButton tooltip={noTooltips ? null : fitScreen ? '原始大小' : '适应内容'} variant={fitScreen ? 'solid' : 'outlined'} onClick={() => setFitScreen(on => !on)}>
                    <FitScreenIcon />
                  </OverlayButton>
                )}
              </ButtonGroup>
            ...
            <ButtonGroup aria-label='Text and code options' sx={overlayGroupWithShadowSx}>
              ...
              {/* Fullscreen */}
              <OverlayButton tooltip={noTooltips ? null : isFullscreen ? '退出全屏' : !renderSyntaxHighlight ? '全屏' : '演示'} variant={isFullscreen ? 'solid' : 'outlined'} onClick={isFullscreen ? exitFullscreen : enterFullscreen}>
                <ZoomOutMapIcon sx={{ fontSize: 'xl' }} />
              </OverlayButton>

              {/* Soft Wrap toggle */}
              {renderSyntaxHighlight && (
                <OverlayButton tooltip={noTooltips ? null : '自动换行'} disabled={!renderSyntaxHighlight} variant={(showSoftWrap && renderSyntaxHighlight) ? 'solid' : 'outlined'} onClick={() => setShowSoftWrap(!showSoftWrap)}>
                  <WrapTextIcon />
                </OverlayButton>
              )}
              ...
              {renderSyntaxHighlight && uiComplexityMode !== 'minimal' && (
                <OverlayButton tooltip={noTooltips ? null : '显示行号'} disabled={cannotRenderLineNumbers} variant={(renderLineNumbers && renderSyntaxHighlight) ? 'solid' : 'outlined'} onClick={() => setShowLineNumbers(!showLineNumbers)}>
                  <NumbersRoundedIcon />
                </OverlayButton>
              )}

              {/* Artifact Dual-pane Preview */}
              <OverlayButton
                tooltip={noTooltips ? null : '在右侧预览'}
                variant='outlined'
                color='success'
                onClick={() => useArtifactsStore.getState().openArtifact({ title: blockTitle || inferredCodeLanguage || 'code', type: inferredCodeLanguage || 'text', content: code })}
              >
                <VerticalSplitIcon />
              </OverlayButton>
              ...
              {hasExternalButtons && (
                <Dropdown>
                  <Tooltip disableInteractive arrow placement='top' title='网页编辑器'>
                    <MenuButton
                  ...
                  <Menu sx={{ minWidth: 160 }} placement='bottom-end'>
                    <ListItem>
                      <Typography level='body-sm'>编辑工具:</Typography>
                    </ListItem>
                    {openExternallyItems}
                  </Menu>
                </Dropdown>
              ...
              {props.noCopyButton !== true && (
                <OverlayButton tooltip={noTooltips ? null : '复制代码'} variant='outlined' onClick={handleCopyToClipboard}>
                  <ContentCopyIcon />
                </OverlayButton>
              )}
            </ButtonGroup>
