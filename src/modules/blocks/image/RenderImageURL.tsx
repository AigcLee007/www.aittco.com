    ...
    <Box>
      ...
      >
        ...
        {!props.disabled && <Box className={overlayButtonsClassName} sx={overlayButtonsGridSx}>
          ...
          {!!props.expandableText && (
            <OverlayButton tooltip={infoOpen ? '隐藏提示词' : '显示提示词'} variant={infoOpen ? 'solid' : 'outlined'} color={isCard ? 'primary' : undefined} onClick={handleToggleInfoOpen} sx={{ gridRow: '1', gridColumn: '1' }}>
              <InfoOutlinedIcon />
            </OverlayButton>
          )}
          ...
          {!!props.imageURL && (
            props.onViewImage ? (
              <OverlayButton tooltip='查看图片' variant='outlined' color={isCard ? 'primary' : undefined} onClick={handleViewImage} sx={{ gridRow: '1', gridColumn: '2' }}>
                <ZoomOutMapIcon />
              </OverlayButton>
            ) : props.imageURL.startsWith('http') ? (
              <OverlayButton tooltip='下载图片' variant='outlined' color={isCard ? 'primary' : undefined} component={Link} href={props.imageURL} download={props.expandableText || 'Image'} target='_blank' sx={{ gridRow: '1', gridColumn: '2' }}>
                <ZoomOutMapIcon />
              </OverlayButton>
            ) : <span />
          )}
          ...
          {deleteArmed && !regenArmed && (
            <OverlayButton tooltip='确认删除' placement='bottom' variant='outlined' color='danger' onClick={onImageDelete} sx={{ gridRow: '2', gridColumn: '1' }}>
              <DeleteForeverIcon sx={{ color: 'danger.solidBg' }} />
            </OverlayButton>
          )}

          {!!onImageDelete && !regenArmed && (
            <OverlayButton tooltip={deleteArmed ? '取消删除' : '删除图片'} placement='bottom' variant={deleteArmed ? 'solid' : 'outlined'} color={isCard ? 'primary' : undefined} onClick={handleToggleDeleteArmed} sx={{ gridRow: '2', gridColumn: '2' }}>
              {deleteArmed ? <CloseRoundedIcon /> : <DeleteOutlineIcon />}
            </OverlayButton>
          )}

          {!!onImageRegenerate && !deleteArmed && (
            <OverlayButton tooltip={regenArmed ? '取消重新生成' : '使用当前配置重新生成'} placement='bottom' variant={regenArmed ? 'solid' : 'outlined'} onClick={handleToggleRegenArmed} sx={{ gridRow: '2', gridColumn: '1' }}>
              {regenArmed
                ? <CloseRoundedIcon />
                : <ReplayIcon />
              }
          ...
          {regenArmed && !deleteArmed && (
            <OverlayButton tooltip='确认重新生成' placement='bottom' variant='outlined' color='success' onClick={handleImageRegenerate} sx={{ gridRow: '2', gridColumn: '2' }}>
      ...
      {isTempDalleUrl && showDalleAlert && (
        ...
        >
          <div>
            <strong>请保存到本地</strong> · OpenAI 将在创建一小时后从其服务器删除此图片链接。
          </div>
        </Alert>
      )}
