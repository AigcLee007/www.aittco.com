...
import { useSmoothTypewriter } from '~/common/hooks/useSmoothTypewriter';
...
export function AutoBlocksRenderer(props: {
  ...
  // Apply smooth typewriter effect for assistant's streaming messages
  // We use props.optiAllowSubBlocksMemo as a proxy for the 'streaming' state
  const isStreaming = fromAssistant && props.optiAllowSubBlocksMemo === false;
  const smoothText = useSmoothTypewriter(props.text, isStreaming);

  // state
  const { text, isTextCollapsed, forceTextExpanded, handleToggleExpansion } =
    useTextCollapser(smoothText, fromUser);
