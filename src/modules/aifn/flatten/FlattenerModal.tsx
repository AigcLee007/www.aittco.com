...
function FlatteningProgress(props: { llmLabel: string, partialText: string | null }) {
  return (
    <Box sx={{ mx: 'auto', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
      <CircularProgress />
      <Typography>
        {props.partialText?.length ? `${props.partialText.length} 字符` : '压缩中...'}
      </Typography>
      <Typography level='body-sm'>
        这可能需要长达一分钟的时间。
      </Typography>
...
export function FlattenerModal(props: {
  ...
  const [llm, llmComponent] = useFormRadioLlmType('模型', runModelId ?? null, 'util');
  const {
  ...
  return (
    ...
    >
      ...
      {confirmOverwrite && <ConfirmationModal
        ...
        confirmationText='您确定要用压缩后的文本覆盖当前对话吗？'
        positiveActionText='替换对话'
      />}
