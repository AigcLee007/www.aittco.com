...
export function DiagramsModal(props: { config: DiagramConfig, onClose: () => void; }) {
  ...
  const [diagramLanguage, languageComponent, setDiagramLanguage] = useFormRadio<DiagramLanguage>('mermaid', diagramLanguages, '语法');
  ...
  const handleGenerateNew = React.useCallback(async () => {
    ...
    if (!diagramType || !diagramLanguage || !diagramLlm)
      return setErrorMessage(`图表类型、语言或模型无效 (${diagramLlm})。`);

    setErrorMessage(null);

    let diagramCode: string = '加载中...';
    ...
    if (!chatSystemInstruction)
      return setErrorMessage('当前对话中没有系统指令');
    ...
    try {
      ...
      setDiagramCode(null);
      setErrorMessage(error?.name !== 'AbortError' ? error?.message : '已中断。');
    } finally {
  ...
  const handleAppendMessageAndClose = React.useCallback(() => {
    if (!diagramCode)
      return setErrorMessage('没有可添加到对话的内容。');
  ...
  return (
    <GoodModal
      ...
      title={<Box sx={{ flex: 1, display: 'flex', alignItems: 'center' }}>
        <AppBreadcrumbs size='md' rootTitle='创建'>
          <AppBreadcrumbs.Leaf><b>图表</b></AppBreadcrumbs.Leaf>
        </AppBreadcrumbs>
        <Box sx={{ ml: 1.25 }}>
          <ChipToggleButton
            text={showOptions ? '收起' : '展开'}
    ...
    >
      ...
      {showOptions && (
        <Grid container spacing={2}>
          ...
          <Grid xs={12} md={6}>
            <FormControl>
              <FormLabel>自定义</FormLabel>
              <Input
                title='自定义指令'
                placeholder='例如：以状态图形式展示'
      ...
      <Box sx={{ mt: 'auto', display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between' }}>
        ...
        <ButtonGroup variant='solid' color='primary' sx={{ ml: 'auto' }}>
          ...
          >
            {abortController ? '停止' : diagramCode ? '重新生成' : '生成'}
          </Button>
        </ButtonGroup>
