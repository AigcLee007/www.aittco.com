...
import { geminiGenerateImagesOrThrow } from './geminiGenerateImages';
...
export async function t2iGenerateImagesOrThrow(
  ...
  switch (vendor) {
    ...
    case 'googleai':
      if (!modelServiceId)
        throw new Error(`No ${vendor} Model service configured for TextToImage`);
      return await geminiGenerateImagesOrThrow(provider, prompt, count, options);
...
function _findLlmsT2IServices(llms: ReadonlyArray<DLLM>, services: ReadonlyArray<DModelsService>) {
  return services
    .filter(s => ['azure', 'openai', 'localai', 'googleai'].includes(s.vId))
