...
// configuration
const BACKUP_FILE_FORMAT = 'AIGC-Club Flash File';
...
async function restoreIndexedDB(allDbData: Record<string, any>): Promise<void> {
  ...
  const dbTargetVersions: { [dbName: string]: number } = {
    'keyval-store': 1,
    'AIGC-Club': 10, // Dexie multiplied the version (1) by 10 (https://github.com/dexie/Dexie.js/issues/59)
  };
          ...
          openRequest.onupgradeneeded = (event) => {
            ...
            if (dbName === 'keyval-store') {
              ...
              if (!db.objectStoreNames.contains('largeAssets')) {
                ...
                logger.info(`Created largeAssets object store with all indices in AIGC-Club database`);
              }
            } else {
...
/**
 * Backup and Restore (Flashing) functionality for AIGC-Club client-side data.
...
export function FlashRestore(props: { unlockRestore?: boolean }) {
  ...
  const handleRestoreLoad = React.useCallback(async () => {
    ...
    try {
      ...
      if (data.metadata.application !== 'AIGC-Club' && data.metadata.application !== 'Big-AGI') {
        // User selected incompatible file - this is expected, not a system error
        setRestoreState('error');
        setErrorMessage(`Incompatible Flash file. Found application "${data.metadata.application}" but expected "AIGC-Club".`);
      ...
      if (backupSchemaVersion > currentSchemaVersion)
        setSchemaVersionWarning(`WARNING: You are restoring from an newer AIGC-Club version to this one. This is a DOWNGRADE and may cause data loss or application errors.`);
      else {
  ...
  return <>
    ...
    >
      {restoreState === 'success' ? '恢复完成' : restoreState === 'error' ? '恢复失败' : restoreState === 'processing' ? '正在恢复...' : '从文件恢复'}
    </Button>
    ...
    >
      <Typography textColor='text.secondary'>
        这将使用所选文件中的内容<Typography fontWeight='lg' color='danger'>替换当前所有应用数据</Typography>。<Typography fontWeight='lg' color='danger'>警告: 这是一个破坏性操作，可能会导致应用无法使用。</Typography>
      </Typography>
      ...
      {backupDataForRestore?.metadata && (
        <Box sx={{ mt: 1, p: 1.5, bgcolor: 'background.level1', borderRadius: 'sm', border: '1px solid', borderColor: 'neutral.outlinedBorder', fontSize: 'sm' }}>
          <Box fontWeight='md' mb={1}>Flash 文件详情:</Box>
          <Divider sx={{ my: 1 }} />
          创建于: {new Date(backupDataForRestore.metadata.timestamp).toLocaleString()}<br />
          备份类型: {backupDataForRestore.metadata.backupType}<br />
          版本: {backupDataForRestore.metadata.version}<br />
          架构版本: {backupDataForRestore.schemaVersion || '未知'}<br />
          租户: {backupDataForRestore.tenantSlug || '未知'}<br />
          <Divider sx={{ my: 1 }} />
          完整数据库: {Object.keys(backupDataForRestore.storage.indexedDB || {}).length}<br />
          设置组: {Object.keys(backupDataForRestore.storage.localStorage).length}<br />
        </Box>
      )}
          ...
          <FormControl orientation='horizontal' sx={{ gap: 1, flex: 1 }}>
            ...
            <FormLabel sx={{ fontWeight: 'sm', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', opacity: hasLocalStorageData ? 1 : 0.5 }}>
              ...
              <Typography level='body-xs' sx={{ fontWeight: 'normal', color: 'text.secondary' }}>
                {hasLocalStorageData ? '(偏好设置, 模型)' : '(不在备份文件中)'}
              </Typography>
            </FormLabel>
          </FormControl>
          <FormControl orientation='horizontal' sx={{ gap: 1, flex: 1 }}>
            ...
            <FormLabel sx={{ fontWeight: 'sm', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', opacity: hasIndexedDBData ? 1 : 0.5 }}>
              对话
              <Typography level='body-xs' sx={{ fontWeight: 'normal', color: 'text.secondary' }}>
                {hasIndexedDBData ? '(聊天记录, 附件)' : '(不在备份文件中)'}
              </Typography>
            </FormLabel>
          </FormControl>
        </Sheet>
...
export function FlashBackup(props: {
  ...
  return <>
    ...
    <Typography level='body-sm' mt={3}>
      保存<strong>所有设置和对话</strong>:
    </Typography>
    <Button
      variant='soft'
      aria-label='下载完整 Flash 文件'
