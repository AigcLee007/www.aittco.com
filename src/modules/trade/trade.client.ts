...
export async function openConversationsAtRestPicker(): Promise<FileWithHandle[] | null> {
  try {
    return await fileOpen({
      ...
      mimeTypes: ['application/json', 'application/aigc-club'],
      multiple: true,
    });
  } catch (error) {
...
/**
 * Download specific multiple conversations as a JSON file
 */
export async function downloadSpecificJsonV1B(conversationIds: string[]) {
  const { folders, enableFolders } = useFolderStore.getState();
  ...
  await fileSave(blob, {
    fileName: `backup_selected_chats_${payload.conversations.length}_${prettyTimestampForFilenames(false)}.agi.json`,
    extensions: ['.json'],
  }).catch(() => null);
}
