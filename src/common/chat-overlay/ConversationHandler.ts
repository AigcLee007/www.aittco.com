...
import { DConversationId, DConversation } from '~/common/stores/chat/chat.conversation';
...
export class ConversationHandler {
  ...
  conversationView(): DConversation {
    const conversation = useChatStore.getState().conversations.find(c => c.id === this.conversationId);
    if (!conversation)
      throw new Error(`ConversationHandler: Conversation not found for ${this.conversationId}`);
    return conversation;
  }
  ...
  setAutoTitle(title: string): void {
    _chatStoreActions.setAutoTitle(this.conversationId, title);
  }
