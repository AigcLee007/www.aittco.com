...
const styles: Record<string, React.CSSProperties> = {
  container: {
    ...
    minHeight: 'calc(100vh * var(--app-inv-zoom))',
    // layout
    display: 'flex',
...
export function ProviderBackendCapabilities(props: { children: React.ReactNode }) {
  ...
  // fetch capabilities
  const { data, isError, error } = apiQuery.backend.listCapabilities.useQuery(undefined, {
    ...
    retry: false, // Do not retry indefinitely to allow showing error UI
  });


  // [effect] log error for debugging
  React.useEffect(() => {
    if (isError) {
      console.error('Failed to fetch backend capabilities:', error);
      setBackendTimeout(true); // Treat error as timeout to show error UI
    }
  }, [isError, error]);
  ...
  // Backend timeout or connection error notice
  if ((backendTimeout || isError) && versionVerified !== true) {
    return (
      <div style={styles.container}>
        <div style={styles.content2}>
          <h2 style={styles.heading}>Connection Error</h2>
          <h2 style={styles.message}>
            {isError ? 'The server returned an error during initialization.' : 'Unable to connect to the server.'}
            <br /><br />
          </h2>
          <button
            style={styles.button}
