...
import { createDMessageDataInlineText, createDocAttachmentFragment, DMessageAttachmentFragment, DMessageDataInline, DMessageDocMimeType, DMessageDocPart, DVMimeType, isContentOrAttachmentFragment, isDocPart, specialContentPartToDocAttachmentFragment } from '~/common/stores/chat/chat.fragments';

import type { AttachmentCreationOptions, AttachmentDraft, AttachmentDraftConverter, AttachmentDraftId, AttachmentDraftInput, AttachmentDraftSource, AttachmentDraftSourceOriginFile, DraftEgoFragmentsInputData } from './attachment.types';
...
export async function attachmentLoadInputAsync(source: Readonly<AttachmentDraftSource>, edit: (changes: Partial<Omit<AttachmentDraft, 'outputFragments'>>) => void) {
  ...
  switch (sourceMedia) {
    ...
    case 'url':
      ...
      edit({ inputError: 'Web page/YouTube import has been disabled for simplification.' });
      break;
  ...
  switch (true) {
    ...
    case mimeTypeIsPlainText(input.mimeType):
      ...
      if (textOriginHtml && isHtmlTable)
        converters.push({ id: 'rich-text-table', name: 'Markdown 表格' });

      // p2: Text
      if (fromCloud && input.mimeType === 'text/markdown') {
        converters.push({ id: 'text', name: 'Markdown' });
      } else {
        converters.push({ id: 'text', name: attachmentSourceSupportsLiveFile(source) ? '文本 (实时)' : '文本' });
        if (!textOriginHtml && textOriginClipboard) {
          converters.push({ id: 'text-markdown', name: '文本 -> Markdown' });
          converters.push({ id: 'text-cleaner', name: '文本 -> 纯净 HTML' });
        }
      }

      // p3: Html -> Markdown, and Html
      if (textOriginHtml) {
        converters.push({ id: 'rich-text', name: 'HTML · 完整' });
        converters.push({ id: 'rich-text-markdown', name: 'HTML -> Markdown' });
        converters.push({ id: 'rich-text-cleaner', name: 'HTML -> 纯净 HTML' });
      }
      break;
    ...
    case input.mimeType.startsWith('image/'):
      ...
      converters.push({ id: 'image-resized-high', name: '图片 (高精度)', disabled: !inputImageMimeSupported });
      converters.push({ id: 'image-resized-low', name: '图片 (低精度)', disabled: !inputImageMimeSupported });
      converters.push({ id: 'image-original', name: '图片 (原始质量)', disabled: !inputImageMimeSupported });
      if (!inputImageMimeSupported)
        converters.push({ id: 'image-to-default', name: `作为图片 (${PLATFORM_IMAGE_MIMETYPE})` });
      converters.push({ id: 'image-caption', name: 'AI 描述 (文本)', disabled: visionModelMissing });
      converters.push({ id: 'unhandled', name: '无图片' });
      converters.push({ id: 'image-ocr', name: '添加文本 (OCR)', isCheckbox: true });
      break;

    // PDF
    case mimeTypeIsPDF(input.mimeType):
      converters.push({ id: 'pdf-auto', name: '自动', isActive: !autoAddImages });
      converters.push({ id: 'pdf-text', name: 'PDF 文本' });
      converters.push({ id: 'pdf-images-ocr', name: 'PDF -> OCR (扫描件)' });
      converters.push({ id: 'pdf-images', name: 'PDF -> 图片' });
      converters.push({ id: 'pdf-text-and-images', name: 'PDF -> 文本 + 图片', isActive: autoAddImages });
      break;

    // DOCX
    case mimeTypeIsDocX(input.mimeType):
      converters.push({ id: 'docx-to-html', name: 'DOCX 转 HTML' });
      break;
    ...
    default:
      ...
      converters.push({ id: 'text', name: '作为文本' });
      break;
  }
...
function _prepareDocData(source: AttachmentDraftSource, input: Readonly<AttachmentDraftInput>, converterName: string): {
  ...
  switch (source.media) {
    ...
    case 'url':
      return {
        title: `Web page: ${source.refUrl}`,
        caption: converterName,
        refString: humanReadableHyphenated(source.refUrl),
      };
        ...
        switch (converter.id) {
          ...
          case 'text-markdown':
            try {
              ...
              textContent = await convertHtmlToMarkdown(textContent);
      ...
      case 'rich-text-markdown':
        ...
        switch (converter.id) {
          ...
          case 'rich-text-markdown':
            try {
              const { convertHtmlToMarkdown } = await import('./file-converters/HtmlToMarkdown');
              richText = await convertHtmlToMarkdown(richText);
        ...
        try {
          ...
          if (pdfTextResult.avgCharsPerPage >= PDF_LOW_TEXT_THRESHOLD) {
            ...
            try {
              ...
              const truncNote = pdfTextResult.pageCount > pagesToProcess ? ` (${pagesToProcess}/${pdfTextResult.pageCount} pages)` : '';
              edit(attachment.id, {
                outputsHeuristic: { isAuto: true, actualConverterId: 'pdf-images', explain: `OCR failed${truncNote}` },
              });
            }
          }
        } catch (error) {
          console.log('[DEV] PDF Auto Error:', error);
          edit(attachment.id, { outputWarnings: ['PDF Auto processing failed.'] });
        }
        break;
