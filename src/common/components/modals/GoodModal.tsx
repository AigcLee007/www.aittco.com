      ...
      <ModalOverflow>
        ...
        >
          ...
          {!props.noTitleBar && <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            {(hasFullscreenButton || !!props.onClose) && (
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mr: -0.5 }}>
                ...
                {hasFullscreenButton && (
                  <IconButton aria-label={showFullscreen ? '退出全屏' : '进入全屏'} size='sm' onClick={toggleFullscreen} sx={{ my: -1 }}>
                    {showFullscreen ? <CloseFullscreenIcon /> : <OpenInFullIcon sx={{ fontSize: 'md' }} />}
                  </IconButton>
                )}
                {/* optional close button */}
                {!!props.onClose && <ModalClose aria-label='关闭对话框' sx={{ position: 'static', my: -1 }} />}
              </Box>
            )}
          ...
          {(!!props.startButton || showBottomClose) && <Box sx={{
            ...
            {props.startButton}
            {showBottomClose && <Button aria-label='关闭对话框' variant='solid' color='neutral' onClick={(event) => props.onClose?.(event, 'closeClick')} sx={{ ml: 'auto', minWidth: 100 }}>
              {props.closeText || '关闭'}
            </Button>}
          </Box>}
