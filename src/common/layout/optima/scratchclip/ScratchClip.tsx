...
export function ScratchClip() {
  ...
  const handleRestoreSnippet = React.useCallback((item: ClipboardHistoryItem, event: React.MouseEvent) => {
    ...
    copyToClipboard(item.text, '来自历史记录的内容');
  ...
  const handleClearHistory = React.useCallback((event: React.MouseEvent) => {
    ...
    addSnackbar({ message: '剪贴板历史记录已清除。', type: 'info', key: 'clip-clear' });
  }, []);
  ...
  const handleReadClipboard = React.useCallback(async () => {
    ...
    if (!clipboardItems || clipboardItems.length === 0) {
      addSnackbar({ key: 'clipboard-issue', type: 'issue', message: '剪贴板为空或被拒绝访问', overrides: { autoHideDuration: 2000, } });
      return;
    }
      ...
      if (types.includes('text/plain')) {
        try {
          ...
          if (text.trim()) {
            ...
            addSnackbar({ message: '已将剪贴板内容添加到历史记录', type: 'success', key: 'clip-read-success' });
          }
        } catch (error) {
    ...
    <GoodModal
      ...
      title={
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          {/* Clear Button */}
          {history.length > 0 && (
            <Tooltip title='清除所有历史' variant='solid' placement='bottom'>
        ...
        >
          {history.map((item) => (
            <ListItem key={item.id}>
              ...
              {/* > Copy */}
              <ListItemDecorator>
                <Tooltip title='复制到剪贴板' variant='outlined' placement='left'>
                  <IconButton
                    size='sm'
                    // color='primary'
