...
import { GlobalSearchModal } from '~/common/layout/overlays/GlobalSearchModal';
...
export function Modals(props: { suspendAutoModelsSetup?: boolean }) {
  ...
  // external state
  const { preferencesTab, showAIXDebugger, showKeyboardShortcuts, showLogger, showPreferences, showModels, showModelOptions, showSearch } = useOptimaModals();

  // derived state
  const { closeAIXDebugger, closeKeyboardShortcuts, closeLogger, closePreferences, closeSearch, openKeyboardShortcuts } = optimaActions();
  ...
  return <>
    ...
    {/* Global Search */}
    {showSearch && (
      <GlobalSearchModal open={showSearch} onClose={closeSearch} />
    )}
