...
import { BlueprintBackground } from '~/common/components/BlueprintBackground';
...
export function PageWrapper(props: { component: React.ElementType, currentApp?: NavItemApp, isMobile: boolean, children: React.ReactNode }) {
  ...
  if (props.isMobile)
    return (
      <Box sx={{ position: 'relative', minHeight: 'calc(100dvh * var(--app-inv-zoom))' }}>
        <BlueprintBackground />
  ...
  return (
    ...
    <Box
      sx={{
        ...
        flex: '1 1 0px',
        overflow: 'hidden',
        minHeight: 'calc(100dvh * var(--app-inv-zoom))',
