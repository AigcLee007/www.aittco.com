...
export type DMessageDocMimeType =
...
function _duplicate_Part<TPart extends (DMessageContentFragment | DMessageAttachmentFragment | DMessageVoidFragment)['part']>(part: TPart): TPart {
  ...
  switch (pt) {
    case 'doc':
      ...
      const newData = _duplicate_InlineData(part.data);
      return _create_Doc_Part(part.vdt, newData, part.ref, part.l1Title, newDocVersion, part.meta ? { ...part.meta } : undefined) as TPart;
  ...
  if (isContentFragment(fragment)) {
    ...
    switch (pt) {
      case 'doc':
        ...
        const mimeType = 'mimeType' in part.data ? part.data.mimeType : 'text/plain';
