...
export async function reconfigureBackendModels(lastLlmReconfigHash: string, setLastReconfigHash: (hash: string) => void, remoteServices: boolean, existingServices: boolean) {
  ...
  // Backend capabilities are not ready yet: return and allow retry later in this session.
  if (!backendReconfigHash) {
    _isConfiguring = false;
    return;
  }

  // No backend changes and models already present: no need to reconfigure again this session.
  if (lastLlmReconfigHash === backendReconfigHash && !initiallyEmpty) {
    _isConfiguring = false;
