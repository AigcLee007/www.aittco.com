...
import { coinGuardMiddleware } from './middleware/coinGuardMiddleware';
 ...
 */
export const adminProcedure = t.procedure.use(authMiddleware).use(async ({ ctx, next }) => {
 ...
 *
 * Now protected by authentication and coin balance check.
 */
export const edgeProcedure = t.procedure.use(authMiddleware).use(coinGuardMiddleware);
