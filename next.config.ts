...
try {
  ...
  if (!buildHash) {
    ...
    if (gitResult.status === 0)
      buildHash = gitResult.stdout.trim();
  }
} catch {
  // final fallback
  buildHash = '2-dev';
...
let nextConfig: NextConfig = {
  ...
  // Skip TypeScript type checking during production build (types are checked in dev)
  typescript: {
...
if (process.env.POSTHOG_API_KEY && process.env.POSTHOG_ENV_ID) {
  console.log(' 🧠 \x1b[1mAIGC-Club\x1b[0m: building with PostHog issue reporting and source maps...');
